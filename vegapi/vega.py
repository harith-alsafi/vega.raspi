...
class Vega(VegaTools, VegaApi, Database):
    devices: List[Device]
    def __init__(self, onRunTools: Callable[[list[RunTool]], RunToolsReturnType], onGetDevices: Callable[[Optional[list[str]]], GetDevicesReturnType], onEveryPeriod: Callable[[], List[DataSeries]], onReset: Callable, period: int = 2):
        VegaTools.__init__(self)
        VegaApi.__init__(self, onGetTools=self.on_get_tools, onRunTools=onRunTools, onGetDevices=onGetDevices, onReset=onReset)
